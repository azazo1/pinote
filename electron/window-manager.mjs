...
export class WindowManager {
  ...
  open(note) {
    ...
    const window = new BrowserWindow({
      ...bounds,
      height: state.collapsed ? COLLAPSED_HEIGHT : bounds.height,
      minWidth: 240,
    ...
    window.on("closed", () => this.windows.delete(note.id));
  ...
  toggleCollapse(id) {
    ...
    if (collapsed) {
      this.store.updateWindow(id, {
        bounds: { ...state.bounds, ...bounds, height: Math.max(state.bounds.height, bounds.height) },
        collapsed: true,
      });
      window.setMinimumSize(240, COLLAPSED_HEIGHT);
      window.setResizable(false);
      window.setBounds({ ...bounds, height: COLLAPSED_HEIGHT }, true);
    } else {
      this.store.updateWindow(id, { collapsed: false });
      window.setMinimumSize(240, 180);
      window.setBounds({ ...bounds, height: state.bounds.height }, true);
  ...
  restoreSavedPositions() {
    for (const [id, window] of this.windows) {
      ...
      const display = this.findDisplay(state);
      const bounds = this.clampBounds(state.bounds, display.workArea);
      window.setBounds({ ...bounds, height: state.collapsed ? COLLAPSED_HEIGHT : bounds.height }, true);
      window.setMinimumSize(240, state.collapsed ? COLLAPSED_HEIGHT : 180);
  ...
  persistBounds(id, force = false) {
    ...
    this.store.updateWindow(id, {
      ...
      bounds: {
        ...bounds,
        height: state.collapsed ? state.bounds.height : bounds.height,
      },
    });
  }
  ...
  clampBounds(bounds, area) {
    const width = Math.min(bounds.width ?? 256, area.width);
